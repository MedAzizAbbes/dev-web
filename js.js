function calculateScore() {
    ...
    var q19Answer = document.querySelector('input[name="q19"]:checked');
    var q20Answer = document.querySelector('input[name="q20"]:checked');
    var q21Answer = document.querySelector('input[name="q21"]:checked');
    var q22Answer = document.querySelector('input[name="q22"]:checked');

    if (!q1Answer || !q2Answer || !q3Answer || !q4Answer || !q5Answer || !q6Answer || !q7Answer || !q8Answer || !q9Answer || !q10Answer || !q11Answer || !q12Answer || !q13Answer || !q14Answer || !q15Answer || !q16Answer || !q17Answer || !q18Answer || !q19Answer || !q20Answer || !q21Answer || !q22Answer) {
    ...
    var q19Value = q19Answer.value.trim();
    var q20Value = q20Answer.value.trim();
    var q21Value = q21Answer.value.trim();
    var q22Value = q22Answer.value.trim();

    if (q1Value === "Uruguay") { score++; }
    if (q2Value === "Italie") { score++; }
    if (q3Value === "Italie") { score++; }
    if (q4Value === "Uruguay") { score++; }
    if (q5Value === "RFA") { score++; }
    if (q6Value === "Brésil") { score++; }
    if (q7Value === "Brésil") { score++; }
    if (q8Value === "Angleterre") { score++; }
    if (q9Value === "Brésil") { score++; }
    if (q10Value === "RFA") { score++; }
    if (q11Value === "Argentine") { score++; }
    if (q12Value === "Italie") { score++; }
    if (q13Value === "Argentine") { score++; }
    if (q14Value === "Allemagne") { score++; }
    if (q15Value === "Brésil") { score++; }
    if (q16Value === "France") { score++; }
    if (q17Value === "Brésil") { score++; }
    if (q18Value === "Italie") { score++; }
    if (q19Value === "Espagne") { score++; }
    if (q20Value === "Allemagne") { score++; }
    if (q21Value === "France") { score++; }
    if (q22Value === "Argentine") { score++; }
